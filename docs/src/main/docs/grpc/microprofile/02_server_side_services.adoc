...
= gRPC MicroProfile Server-Side Services
...
To build Helidon gRPC MP include the services include the dependency:
[source,xml]
.Maven Dependency
----
<dependency>
...
Protobuf serializable classes but this is not the case with Helidon gRPC. Helidon supports <<../09_marshalling.adoc, pluggable Marshallers>>
and by default will support any Java primitive or Java `Serializable` as well as Protobuf types. Any type that can be marshalled by the built-in
marshallers or custom supplied marshaller may be used as a request or response type.
...
<<../04_service_implementation.adoc,non-microprofile Helidon gRCP services>>.
...
the <<../03_routing.adoc,basic gRPC server documentation>>).
