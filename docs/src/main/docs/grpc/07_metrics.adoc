...
GrpcRouting routing = GrpcRouting.builder()
        .intercept(GrpcMetrics.counted()
                .nameFunction((svc, method, metric) -> "Foo") // <1>
----
In the above example the `NamingFunction` is just a lambda that returns "Foo" which will be used
as the metric name.
...
GrpcRouting routing = GrpcRouting.builder()
        .intercept(GrpcMetrics.counted()
                .nameFunction((svc, method, metric) -> svc.name() + '.' + method + '.' + type) // <1>
----
In the above example the `NamingFunction` creates a name by concatenating the service name, the method
name and the metric type. So for a service "Foo", method "Bar" the above example would produce a name
"Foo.Bar.counted".
