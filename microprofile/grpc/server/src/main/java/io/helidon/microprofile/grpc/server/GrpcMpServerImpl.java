...
public class GrpcMpServerImpl
    ...
    GrpcMpServerImpl(GrpcMpServer.Builder builder, Server webServer) {
        ...
        GrpcRouting routing = builder.routings();

        STARTUP_LOGGER.finest("Obtained gRPC routing(s)");

        grpcServer = GrpcServer.create(serverConfigBuilder.build(), routing);

        STARTUP_LOGGER.finest("gRPC Server created");
    }
    ...
    @Override
    public int httpPort() {
        return webServer == null ? -1 : webServer.port();
    }
